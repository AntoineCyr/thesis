...
% JC: An example would help for templates and signals
...
% JC: If you haven't already give a 1 paragraph description of what a hash function is (collision resistant and pre-image resistant) ... 
% JC: ANY hash function (sponge or not) can take any size input by operating in rounds, sponge is just a modern way to do it that is easier to make security assertions about
...
% JC: Using $a=b+c \bmod q$ would help with the next paragraph. $q$ is a parameter of the elliptic curve Circom uses (configurable)
...
% JC: say more as multiplications can get big fast
...
% JC: small numbers have a lot of leading 0's when represented in binary. Proving an $n$-bit number has $i$ leading zeros proves it is less than $2^n-i$ 
...
%JC: this could be clearer. Your idea -> Circom -> circuit -> SnarkJS -> proof
...
% Also should mention that setup is trusted and think about who is going to do the setup... you could also punt on this and say that you benchmark groth16 but in practice someone might want to use universal setup (plonk) or transparent setup (starks) 
...
% JC: this says "the tree" as if the reader will know there is a tree and what it is... you should back up and explain what the tree is and what it is used for
...
\label{subsec:pl}
The proof of liabilities operates on a list of balances and a list of email hashes as private inputs. %JC: say why in a sentence
...
These verifications are crucial to prevent overflow or underflow issues, given that the operations occur within a finite field. % issues = attacks, worth empahsizing that an insolvent exchange can get away with a proof if it can exploit overflows (maybe an example? easiest way it to encode "negative" ballances which in finite fields are just really big values that will overflow) 
...
\begin{itemize}
   ...
   \item List of email hash (private): User emails, hidden to protect identities and used in the Merkle tree. %JC: used to prevent "clash attacks" where Alice and Bob happen to both have 5.5 BTC on the exchange so the exchange says to Alice, your index is 50294 in the tree and goes to Bob and says the same thing, index 50294. Alice and Bob are happy and the exchange can only include 5.5 BTC instead of 11 BTC on its balance sheet. By including emails, Alice or Bob will recognize the email is not right for them. 
   \end{itemize}
...
% Small range (first bit is 0) => no negative values... I would combine and just say all balances are small, positive values without possibility of overflow under addition. $n$ balances where each balance is $k$ bits and they are added together, then the sum can never exceed $n \cdot (2^k - 1)$

This proof of liabilities operates as intended because it returns the sum of the liabilities, which is exact because of the verifications. %If users verify, that is another issue
